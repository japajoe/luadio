...
static ImGui::ImDragBehaviourCallback dragBehaviourCallback = NULL;

void ImGui::SetDragBehaviourCallback(ImDragBehaviourCallback callback)
{
    dragBehaviourCallback = callback;
}
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (*v == v_cur)
    {
        if(dragBehaviourCallback)
            dragBehaviourCallback(false);
        return false;
    }
    *v = v_cur;

    if(dragBehaviourCallback)
        dragBehaviourCallback(true);
